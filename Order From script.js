...
function validateStep1(){
  ...
  if(parseFloat(_('multiplier').value) <= 0){ _('multiplierError').classList.remove('hidden'); ok = false;} else _('multiplierError').classList.add('hidden');
  return ok;
}
function validateStep2(){
  let ok = true;
  const name = _('fullName').value.trim();
  const contact = _('contact').value.trim();
  const phoneRe = /^01[0-9]{8,9}$/;
  const email = _('email').value.trim();
  const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if(!name){ _('nameError').classList.remove('hidden'); ok=false; } else _('nameError').classList.add('hidden');
  if(!phoneRe.test(contact)){ _('contactError').classList.remove('hidden'); ok=false; } else _('contactError').classList.add('hidden');
  if (!emailRe.test(email)){ _('emailError').classList.remove('hidden'); ok=false; } else _('emailError').classList.add('hidden');
  return ok;
}
function validateStep3(){
  ...
  if(!(_('passportFile').files && _('passportFile').files[0])){ _('fileError').classList.remove('hidden'); ok=false; } else _('fileError').classList.add('hidden');
  return ok;
}
...
function setActiveStep(n){
  ['s1','s2','s3'].forEach((id,i)=>{
    ...
    else { el.style.background='#f3f4f6'; el.style.color='#374151'; }
  });
...
function goToStep(n){
  ...
  if(n===2 && !validateStep1()){ showNotif('⚠️ Step1 ভুল আছে', true); return; }
  if(n===3 && !validateStep2()){ showNotif('⚠️ Step2 ভুল আছে', true); return; }
  setActiveStep(n);
}

/* ---------- confirm order (send to server) ---------- */
async function confirmOrder(){
  if(!window._lastCalc){
    showNotif('⚠️ Click Calculate in Step 1', true);
    setActiveStep(1);
    return;
  }
  if(!validateStep3()){
  ...
  fd.append('email', _('email').value.trim());            // ⭐ Email added
  fd.append('dob', _('dob').value);
  fd.append('company', _('Company').value.trim());
  fd.append('country', _('countrySearch').value.trim());
  fd.append('occupation', _('occupation').value.trim());
  fd.append('address', _('address').value.trim());
  ...
  try {
    ...
    if(j.success){
      showNotif(`✅ Thank you! your order has been completed. 
      To make payment, please contact this number +880 1851-008300
      . Order ID: ${orderId}`, false, 35000);
      setTimeout(()=> window.location.href = "", 35000);
    } else {
      showNotif('❌ সার্ভারে সমস্যা — অর্ডার জমা হয়নি', true, 5000);
      console.error(j);
    }

  } catch(err){
    console.error(err);
    showNotif('❌ Network / Server error — আবার চেষ্টা করুন', true, 5000);
  }
}
...
if(!emailRe.test(email)){
  _('emailError').classList.remove('hidden');
  ok=false;
} else _('emailError').classList.add('hidden');
...
document.getElementById("confirmOrderBtn").addEventListener("click", async function (e) {
    e.preventDefault();
        ...
        if (result.isConfirmed) {
            ...
            if (data.success) {
                Swal.fire({
                    ...
                    html: `Your Order ID: <b>${orderId}</b>`,
                    icon: "success",
                });
            } else {
                Swal.fire({
  ...
  document.getElementById("orderForm").addEventListener("submit", async function(e){
        ...
        Swal.fire({
            icon: "success",
            title: "Order Confirmed!",
            html: `<h3>Your Order ID:</h3><b style="font-size:22px;color:#1a73e8">${orderId}</b>`,
